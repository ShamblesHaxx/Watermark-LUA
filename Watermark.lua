...
local Loop = game:GetService("RunService").RenderStepped:Connect(function(step)
    Watermark.Text.Text = string.format("%s | v%s [%s] | FPS: %s | Ping: %sms", getgenv().Name, getgenv().Version, getgenv().Build, math.floor(1/step), math.floor(game:GetService('Stats').Network.ServerStatsItem["Data Ping"]:GetValue()))
